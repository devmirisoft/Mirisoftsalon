...
const normalizeRegisterInput = (value: unknown) => {
...
export const registerSchema = z.preprocess(normalizeRegisterInput, z.object({
  ...
  password: z
    ...
    .min(
      6,
      "Passcode is too short. Use at least 6 characters. Example: StrongPass123."
    ),
  confirmPassword: z
    .string({ error: "Confirm Passcode is required. Re-enter the same passcode." })
    .min(
      6,
      "Confirm Passcode is too short. Use at least 6 characters. Example: StrongPass123."
    ),
  ...
  message:
    "Confirm Passcode does not match. Example: type the same value as Passcode.",
}));
