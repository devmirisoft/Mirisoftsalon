...
const redactGst = (value) => value ? `${value.slice(0, 2)}***********${value.slice(-2)}` : null;
...
export const createSalon = async (req, res) => {
    try {
        const { name, email, phone, addressLine1, city, state, postalCode, timezone, } = req.body;
        if (!name) {
...
export const getSalonGstSettings = async (req, res) => {
    try {
...
export const updateSalonGstSettings = async (req, res) => {
    try {
        const salonId = getTargetSalonId(req);
        if (!salonId) {
            return res.status(400).json({ success: false, message: "Salon ID is required" });
        ...
        const gstNumber = "gstNumber" in req.body && req.body.gstNumber !== null
            ...
            : "gstNumber" in req.body
                ? null
                : existing.gstNumber;
        const gstLegalName = "gstLegalName" in req.body && req.body.gstLegalName !== null
            ? String(req.body.gstLegalName).trim()
            : "gstLegalName" in req.body
                ? null
                : existing.gstLegalName;
        const gstStateCode = "gstStateCode" in req.body && req.body.gstStateCode !== null
        ...
        const serviceGstRate = "serviceGstRate" in req.body
            ? parseRate(req.body.serviceGstRate, "serviceGstRate")
            : { value: existing.serviceGstRate };
        const productGstRate = "productGstRate" in req.body
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            const saved = await SalonModel.updateGstSettings(salonId, {
                gstEnabled,
                gstNumber,
                gstLegalName,
                gstStateCode,
