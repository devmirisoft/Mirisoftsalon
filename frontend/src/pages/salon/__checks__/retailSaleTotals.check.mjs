...
export const retailTotals = (items, discountPercentInput, membershipPercent, taxPercent) => {
...
assert.deepEqual(retailTotals([{ productId: "a", quantity: 3, price: 150 }], "10", 0, 18), {
  subtotal: 450,
  manualDiscount: 45,
  total: 477.9,
});
...
assert.equal(retailTotals([{ productId: "", quantity: 1, price: 999 }, { productId: "a", quantity: 1, price: 100 }], 50, 0, 0).manualDiscount, 50);

// Out-of-range input clamps, so the API never sees a discount above the subtotal.
assert.equal(retailTotals([{ productId: "a", quantity: 1, price: 80 }], 250, 0, 0).manualDiscount, 80);
assert.equal(retailTotals([{ productId: "a", quantity: 1, price: 80 }], -5, 0, 0).manualDiscount, 0);

// Membership only takes what the manual discount left.
assert.equal(retailTotals([{ productId: "a", quantity: 1, price: 100 }], 90, 20, 0).total, 0);
